...
class Manus_WP_Reposter_Translator {
    ...
    /**
     * Verifica se o tradutor está funcionando corretamente
     */
    ...
    /**
     * Versão segura de detecção de idioma que não falha
     */
    public function detect_language_safe( $text ) {
        try {
            return $this->detect_language( $text );
        } catch ( Exception $e ) {
            // Fallback simples
            return 'en';
        }
    ...
    private function translate_with_deepl( $text, $source_lang, $target_lang ) {
        ...
        // LOG DE DEBUG
            ...
            error_log( 'API Key exists: ' . ( ! empty( $this->api_key ) ? 'YES' : 'NO' ) );
            error_log( 'Text length: ' . strlen( $text ) );
            error_log( 'Source lang: ' . $source_lang );
            error_log( 'Target lang: ' . $target_lang );
